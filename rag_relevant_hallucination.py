...
_ = vector_store.add_documents(doc_splits)
...
# def prompt_to_ans(generations):
#     for message in generations["messages"]:
...
#  从 State 中提取 answer
def prompt_to_ans(state):
    for message in state["generate"]["messages"]:
        if isinstance(message, AIMessage):
            return message.content
    return None  
